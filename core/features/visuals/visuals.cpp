...
void c_visuals::player_rendering(player_t* entity) noexcept {
	...
	if (c_config::get().player_health) {
		...
		int r = c_config::get().clr_health[0] * 255;
		int g = c_config::get().clr_health[1] * 255;
		int b = c_config::get().clr_health[2] * 255;
		render::get().draw_filled_rect(temp_bg.x - 1, temp_bg.y - 1, temp_bg.w + 2, temp_bg.h + 2, color(0, 0, 0, 25 + alpha[entity->index()]));
		render::get().draw_filled_rect(temp.x, temp.y, temp.w, temp.h, color(r, g, b));
	}
	...
	if (c_config::get().player_weapon) {
		...
		if (names == "4") {
			render::get().draw_text(bbox.x + (bbox.w / 2), bbox.h + bbox.y + 2, render::get().name_font, n4mes, true, color(red, green, blue, alpha[entity->index()]));
		}
...
void c_visuals::projectiles(player_t* entity) noexcept {
	...
	if (model) {
		...
		int r = c_config::get().projek_clr[0];
		int g = c_config::get().projek_clr[1];
		int b = c_config::get().projek_clr[2];
		...
		if (name.find("flashbang") != std::string::npos) {
			...
			if (!(18 - time < 0)) {
				render::get().draw_filled_rect(grenade_position.x - 19, grenade_position.y + 14, 36, 3, color(5, 5, 5, 255));
				render::get().draw_filled_rect(grenade_position.x - 18, grenade_position.y + 13, time * 2, 3, color(255, 255, 255, 255));
			}
		}
...
void c_visuals::skeleton(player_t* entity) noexcept {
	...
	for (int i = 0; i < p_studio_hdr->bones_count; i++) {
		...
		if (bone && (bone->flags & BONE_USED_BY_HITBOX) && (bone->parent != -1)) {
			...
			int r = c_config::get().skelecolor[0] * 255;
			int g = c_config::get().skelecolor[1] * 25;
			int b = c_config::get().skelecolor[2] * 25;

			if (c_math::get().world_to_screen(v_parent, s_parent) && c_math::get().world_to_screen(v_child, s_child))
			{
				render::get().draw_line(s_parent[0], s_parent[1], s_child[0], s_child[1], color(r, g, b, alpha[entity->index()]));
			}
		}
	}
}

void c_visuals::skeletondebug(player_t* entity) noexcept {
