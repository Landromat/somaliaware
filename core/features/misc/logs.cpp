...
void c_event_logs::event_item_purchase(i_game_event* event) noexcept {
	...
	std::stringstream ss;
	ss << "[SamoliaWare] " << player_name << " purchased a " << event->get_string("weapon");

	utilities::console_warning("[player purchase] ");
	interfaces::console->console_printf("%s purchased a %s.     \n", player_name.c_str(), event->get_string("weapon"));

 	//printf("[EthiopianSense] ", player_name, " purchased a ", event->get_string("weapon"));

	add(ss.str(), color(8, 214, 138, 255));
...
void c_event_logs::event_player_hurt(i_game_event* event) noexcept {
	...
	if (attacker == local_player) {
		...
		ss << "[SamoliaWare]" << " hit " << player_name.c_str() << " in the " << hitgroup << " for " << damage << " damage";
		...
		//printf("[EthiopianSense] ", " hit ", player_name.c_str(), " in the ", hitgroup, " for ", damage, " damage", ss, " (", health, " health remaining).");
		//player info
